...
if (!class_exists('miw_widget_multiple_images')) {
	class miw_widget_multiple_images extends WP_Widget{
		...
		public function __construct() {
			...
			add_action( 'wp_enqueue_scripts', array( $this, 'miw_enqueue_style_and_script' ));
		}
		...
		public function miw_widget_multiple_image_script() {
		    wp_enqueue_script( 'image', plugin_dir_url( __FILE__ ). 'js/amiw-widget.js', array( 'jquery' ), '1.1', false );
			wp_enqueue_script( 'image' );
		}

		public function miw_enqueue_style_and_script() {
			wp_register_style( 'swiper-css', plugin_dir_url( __FILE__ ). 'css/swiper.min.css', null, '4.5.0', false );
			wp_register_style( 'custom-css', plugin_dir_url( __FILE__ ). 'css/custom.css', null, '4.5.0', false );

			wp_enqueue_script( 'swiper-js', plugin_dir_url( __FILE__ ). 'js/swiper.min.js', array( 'jquery' ), '4.5.0', false );
			wp_enqueue_script( 'custom-js', plugin_dir_url( __FILE__ ). 'js/slider.js', array( 'jquery' ), '1.1', 99 );
		...
		public function widget( $args, $instance ) {
			...
			$active_slider = !empty( $instance['active_slider'] ) ? $instance['active_slider'] : '';
		...
		public function form( $instance ) {
			$active_slider = '';
			if(isset($instance['active_slider'])){
				$active_slider = ($instance['active_slider'] == 'on') ? 'checked="checked"' : '';
			}
		...
		<p>
			...
			<?php if( !empty( $instance['widget_image'] ) ) { ?>
				...
				<p>
					<input class="checkbox" id="<?php echo $this->get_field_id( 'active_slider' ); ?>" size="3"  name="<?php echo $this->get_field_name( 'active_slider' ); ?>" type="checkbox" <?php echo $active_slider; ?> />
					<label for="<?php echo $this->get_field_id( 'active_slider' ); ?>"><?php esc_html_e( 'If You Want Slider Then Select.', 'multiple-images-widget' ); ?></label> 
				</p>
			<?php } ?>
			<?php
		}
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['active_slider'] = ( ! empty( $new_instance['active_slider'] ) ) ? strip_tags( $new_instance['active_slider'] ) : '';
